...
function buildTypes() {
	...
	host.writeFile = (file, content) => {
		...
		dts += "declare global {\n";
		dts += "\timport { KaboomCtx } from \"types\";\n"

		const processStmt = (stmt) => {

			if (stmt.kind === "ModuleDeclaration") {
				stmt.body.statements.forEach(processStmt);
				return
			}
	...
	const program = ts.createProgram(["src/kaboom.ts"], options, host);
	...
	if (res.emitSkipped) {
		throw new Error("Failed to generate kaboom.d.ts");
	}
