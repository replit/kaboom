import { vec2, vec3, Vec3, Rect, Line, Circle, Color, Vec2, Mat4, Quad, RNG, quad, rgb, hsl2rgb, rand, randi, randSeed, chance, choose, clamp, lerp, map, mapc, wave, testAreaRect, testAreaLine, testAreaCircle, testAreaPolygon, testAreaPoint, testAreaArea, testLineLineT, testRectRect2, testLineLine, testRectRect, testRectLine, testRectPoint, testPolygonPoint, testLinePolygon, testPolygonPolygon, testCircleCircle, testCirclePoint, testRectPolygon, minkDiff, deg2rad, rad2deg, } from "./math";
import { IDList, downloadURL, downloadBlob, uid, isDataURL, deepEq, } from "./utils";
...
export default (gopt = {}) => {
    ...
    const gfx = (() => {
        ...
        const emptyTex = makeTex(new ImageData(new Uint8ClampedArray([255, 255, 255, 255,]), 1, 1));
        ...
        const bgTex = makeTex(new ImageData(new Uint8ClampedArray([
            128, 128, 128, 255,
            190, 190, 190, 255,
            190, 190, 190, 255,
            128, 128, 128, 255,
    ...
    function loadFont(name, src, gw, gh, opt = {}) {
        return assets.fonts.add(name, loadImg(src)
            .then((img) => {
            return makeFont(makeTex(img, opt), gw, gh, opt.chars ?? ASCII_CHARS);
        }));
    }
    ...
    function loadRawSprite(src, opt = {}) {
    ...
    function loadAseprite(name, imgSrc, jsonSrc) {
        return assets.sprites.add(name, new Promise(async (resolve, reject) => {
            ...
            spr.frames = data.frames.map((f) => {
                return new Quad(f.frame.x / size.w, f.frame.y / size.h, f.frame.w / size.w, f.frame.h / size.h);
            });
    ...
    function loadShader(name, vert, frag, isUrl = false) {
        return assets.shaders.add(name, new Promise((resolve, reject) => {
    ...
    function loadSound(name, src) {
        return assets.sounds.add(name, new Promise((resolve, reject) => {
            if (!src) {
            ...
            if (typeof (src) === "string") {
                fetchURL(src)
                    ...
                    .then((data) => {
                    return new Promise((resolve2, reject2) => audio.ctx.decodeAudioData(data, resolve2, reject2));
                })
                    .then((buf) => {
                    resolve({
                        buf: buf,
    ...
    function makeShader(vertSrc = DEF_VERT, fragSrc = DEF_FRAG) {
        let msg;
    ...
    // TODO: expose
    function drawRaw(verts, indices, fixed, tex = gfx.defTex, shaderSrc = gfx.defShader, uniform = {}) {
        ...
        for (const v of verts) {
            ...
            gfx.vqueue.push(pt.x, pt.y, v.pos.z, v.uv.x, v.uv.y, v.color.r / 255, v.color.g / 255, v.color.b / 255, v.opacity);
        }
    ...
    function frameStart() {
        ...
        if (!gopt.background) {
            drawUVQuad({
                width: width(),
                height: height(),
                quad: new Quad(0, 0, width() * app.scale / BG_GRID_SIZE, height() * app.scale / BG_GRID_SIZE),
                tex: gfx.bgTex,
                fixed: true,
            });
        }
    ...
    function ndc2screen(pt) {
        return vec2((pt.x + 1) / 2 * width(), -(pt.y - 1) / 2 * height());
    }
    ...
    function getArcPts(pos, radiusX, radiusY, start, end, res = 1) {
    ...
    function drawEllipse(opt) {
        ...
        const pts = getArcPts(vec2(0), opt.radiusX, opt.radiusY, start, end, opt.resolution);
        // center
        pts.unshift(vec2(0));
        const polyOpt = {
            ...opt,
    ...
    function applyCharTransform(fchar, tr) {
        ...
        if (tr.scale)
            fchar.scale = fchar.scale.scale(vec2(tr.scale));
        if (tr.angle)
    ...
    function compileStyledText(text) {
        const charStyleMap = {};
        ...
        for (const match of text.matchAll(TEXT_STYLE_RE)) {
            ...
            for (let i = origIdx; i < match.index + match.groups.text.length; i++) {
                charStyleMap[i] = {
                    localIdx: i - origIdx,
                    styles: styles,
                };
    ...
    app.canvas.addEventListener("mousemove", (e) => {
        app.mousePos = vec2((e.offsetX - gfx.viewport.x) * width() / gfx.viewport.width, (e.offsetY - gfx.viewport.y) * height() / gfx.viewport.height);
    ...
    function onClick(tag, cb) {
        if (typeof tag === "function") {
            return onMousePress(tag);
        }
        else {
            return onUpdate(tag, (o) => {
                if (!o.area)
    ...
    function onHover(t, onHover, onNotHover) {
        return onUpdate(t, (o) => {
            if (!o.area)
    ...
    function onMousePress(m, action) {
        if (typeof m === "function") {
    ...
    function onMouseRelease(m, action) {
        if (typeof m === "function") {
    ...
    function regCursor(c, draw) {
        // TODO
    }
    function makeCollision(target, dis) {
        return {
            target: target,
            displacement: dis,
            isTop: () => dis.y > 0,
                ...
                if (this.solid && this.area?.shape === "rect") {
                    ...
                    game.root.every((other) => {
                        ...
                        if (testRectPoint(md, vec2(0))) {
                            let dist = Math.min(Math.abs(md.p1.x), Math.abs(md.p2.x), Math.abs(md.p1.y), Math.abs(md.p2.y));
                            const res = (() => {
                                switch (dist) {
                        ...
                        for (const s in lines) {
                            ...
                            if ((dx === 0 && line.p1.x === 0 && line.p2.x === 0)
                                ||
    ...
    function outview(opt = {}) {
        ...
        return {
            ...
            isOutOfView() {
                ...
                const screenRect = new Rect(vec2(0, 0).sub(offset), vec2(width(), height()).add(offset));
    ...
    function outline(width = 1, color = rgb(0, 0, 0)) {
        return {
            id: "outline",
            outline: {
                width,
    ...
    function timer(time, action) {
        const timers = new IDList();
        if (time && action) {
    ...
    function state(initState, stateList, transitions) {
        ...
        return {
            ...
            onStateDraw(state, action) {
                on("draw", state, action);
            },
            onStateLeave(state, action) {
    ...
    function addLevel(map, opt) {
        ...
        const level = {
            ...
            getPos(...args) {
                const p = vec2(...args);
                return vec2(offset.x + p.x * opt.width, offset.y + p.y * opt.height);
            },
            spawn(sym, ...args) {
                const p = vec2(...args);
                const comps = (() => {
                ...
                const posComp = vec2(offset.x + p.x * opt.width, offset.y + p.y * opt.height);
                for (const comp of comps) {
                    if (comp.id === "pos") {
